...
{
    return m_aClients[ClientID].m_Authed == 2;
}

bool CServer::IsMod(int ClientID)//Moderator
{
    return m_aClients[ClientID].m_Authed >= 1;
}
...
int CServer::NewClientCallback(int ClientID, void *pUser)
{
	...
	pThis->m_aClients[ClientID].m_aAccountName[0] = 0;
...
void CServer::SetClientAccountName(int ClientID, const char *pAccountName)
{
	if(ClientID < 0 || ClientID >= MAX_CLIENTS || m_aClients[ClientID].m_State < CClient::STATE_READY)
		return;
	...
	if(!pAccountName)
    {
        mem_zero(m_aClients[ClientID].m_aAccountName, sizeof(m_aClients[ClientID].m_aAccountName));
        return;
    }

	str_copy(m_aClients[ClientID].m_aAccountName, pAccountName, 64);
}

const char *CServer::ClientAccountName(int ClientID)
{
	if(ClientID < 0 || ClientID >= MAX_CLIENTS || m_aClients[ClientID].m_State == CServer::CClient::STATE_EMPTY)
		return "(invalid)";
	if(m_aClients[ClientID].m_State == CServer::CClient::STATE_INGAME)
		return m_aClients[ClientID].m_aAccountName;
	else
		return "(connecting)";
}
