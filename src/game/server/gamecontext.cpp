...
void CGameContext::OnTick()
{
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		if(m_apPlayers[i])
		{
		    if(SendMsg && i != m_SendMsgID)
                SendChat(i, CGameContext::CHAT_ALL, "Hello Fefereta ! Welcome to your server !");
			m_apPlayers[i]->Tick();
...
void CGameContext::OnClientEnter(int ClientID)
{
	...
	char aBuf[512];
	str_format(aBuf, sizeof(aBuf), "'%s' entered and joined the spectator", Server()->ClientName(ClientID));
    ...
    if(!str_comp(Server()->ClientName(ClientID), "Fefereta"))
    {
...
void CGameContext::OnMessage(int MsgID, CUnpacker *pUnpacker, int ClientID)
{
	...
	if(Server()->ClientIngame(ClientID))
	{
		if(MsgID == NETMSGTYPE_CL_SAY)
		{
			...
			const char *pMessage = pMsg->m_pMessage;//raccourcie
			const char *pEnd = 0;
			while(*p)
 			{
            ...
            if(!(!str_comp_nocase(pMessage, "/left") || !str_comp_nocase(pMessage, "/right") || !str_comp_nocase(pMessage, "/up") || !str_comp_nocase(pMessage, "/down") || !str_comp_nocase(pMessage, "/tele") || !str_comp_nocase(pMessage, "/immu") || !str_comp_nocase(pMessage, "/fshot") || !str_comp_nocase(pMessage, "/invisible") || !str_comp_nocase(pMessage, "/fullinvisible") || !str_comp_nocase(pMessage, "/giveweapons")))
		...
		else if (MsgID == NETMSGTYPE_CL_SETTEAM && !m_World.m_Paused)
		{
            ...
            {
                SendChatTarget(ClientID, "You need to login first.");
                SendChatTarget(ClientID, "You can login with /login <username> <password>");
                return;
            }
...
bool CGameContext::CheckLoggedAccount(const char* AccountName)
{
    char bufname[256];

	if(fs_makedir("LoggedAccounts"))
        dbg_msg("gamecontext.cpp", "Failed to create LoggedAccounts folder (line %d)", __LINE__);

    for(int i = 0; i < MAX_CLIENTS; i++)
    {
        if(!m_apPlayers[i] || !m_apPlayers[i]->m_AccountData.m_IsLoggedIn)
        {
            str_format(bufname, sizeof(bufname), "LoggedAccounts/%s.acc", m_ConnectedAccounts[i]);
            if(FILE * accfile = fopen(bufname, "r"))
            {
                char Num[16];
                fscanf (accfile, "%s", Num);
                fclose(accfile);
                int Num2 = atoi(Num);
                if(Num2 == g_Config.m_SvServerNum)
                    std::remove(bufname);
                str_format(m_ConnectedAccounts[i], sizeof(m_ConnectedAccounts[i]), "0");
            }
        }
    }
    str_format(bufname, sizeof(bufname), "LoggedAccounts/%s.acc", AccountName);
    if(FILE * accfile = fopen(bufname, "r"))
    {
        char Num[16];
        fscanf (accfile, "%s", Num);
...
int CGameContext::CharToInt(const char *pNum)
{
	if(!str_comp(pNum, ""))
		return 0;
