...
CPlayer::CPlayer(CGameContext *pGameServer, int ClientID, int Team)
{
	...
	m_pAccount = new CAccount(this, GameServer());
}
...
void CPlayer::OnDisconnect(const char *pReason)
{
	...
	if(Server()->ClientIngame(m_ClientID))
	{
		char aBuf[512];
		if(pReason && *pReason)
			str_format(aBuf, sizeof(aBuf), "'%s' has left the city (%s)", Server()->ClientName(m_ClientID), pReason);
		else
			str_format(aBuf, sizeof(aBuf), "'%s' has left the city", Server()->ClientName(m_ClientID));
